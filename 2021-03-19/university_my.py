...
def index(request):
    template = load_template('university.html')
...
def university_response(request):
    ...
    mathematics = request.data['mathematics']
    latvian_language = request.data['latvian_language']
    foreign_language = request.data['foreign_language']

    named_university = university.University(
        full_name=full_name,
        mathematics=mathematics,
        latvian_language=latvian_language,
        foreign_language=foreign_language,
    )

    university_check = named_university.point_check()

    if university_check:
        result = "can"
    else:
        result = "can`t"
    content = named_university.generate_report(result)
